"""Build the full-range product catalogue, laid out after a consumer-brand
range brochure: a photographic cover, a "choose the right machine" spread,
hero spreads (a white feature panel beside a full-page photograph with
numbered call-outs), range pages paired with specification charts, a spares
page, a "why us" page and a back cover with a write-in requirement form.

    python brochure/generate_range_catalogue.py            # PDF
    python brochure/generate_range_catalogue.py --html     # also keep the HTML

Run from the repo root. Output: brochure/Manual_Tools_Co_Catalogue.pdf

Pages are A4 portrait, designed as facing pairs (even page left, odd page
right) and 20 pages long, a multiple of four, so the same file prints
single-sided on an office printer or as a folded, stapled booklet.

All product copy comes from PRODUCTS in generate_product_brochures.py and the
four "Key specifications" per machine from product-data.php, so the catalogue
cannot drift from the website or the single-product brochures. Company facts
(since 1995, 30+ years, 150+ clients, 500+ projects, 15+ staff, ISO
9001:2015, the client list) are the ones the website already states.

The page is HTML rendered by headless Chrome (or Edge): Archivo, a variable
font with a width axis, gives the wide heavy italic headlines, and CSS does
the photo overlays that fpdf cannot.
"""
import html
import os
import re
...
ROOT = Path.cwd()
OUT = ROOT / "brochure" / "Manual_Tools_Co_Catalogue.pdf"
...
def build_html(build):
    ...
    n += 2
    pages += [page_spares(img, n), page_why(img, n + 1), page_back(img, make_qr(build))]

    doc = f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<title>Manual Tools Company – Product Range</title>
<style>{css()}</style>
...
def main():
    ...
    build = Path(tempfile.mkdtemp(prefix="mtc-catalogue-"))
    try:
        page, count = build_html(build)
        if count % 4:
            print(f"warning: {count} pages is not a multiple of 4; a folded booklet needs blanks")
