...
- [x] **2026-09-19: Audit of the day's changes.** `check_pages.py` against production: 16 pages, **0 failures**. Live spot-checks confirmed every spec change. One real defect found and fixed: `/coal-charging-car`'s overview still said the car takes coal "into **four** conical hoppers" and lines up with "the oven's **four** charging holes", contradicting the 2/3/4 mouths the specs now state - every sweep during the change had searched for the digit `4`, so the spelled-out word survived. Also removed two leftovers from the brochure rewrite (`brand_bar()` and the unused `BADGE` import).
  - **Still open, for the owner to decide:** `brochure/Manual_Tools_Co_Catalogue.pdf` (catalogue 1, 32 pages, 23 MB) is built and committed but **linked from nowhere** - only `Manual_Tools_Co_Catalogue_2.pdf` is, from the header - so the "At the battery" spread added today is not reachable by any visitor. And `brochure/fonts/fa-solid-900.ttf` (201 KB) is now orphaned: it existed only for fpdf2's icons, and the HTML brochures use `assets/fontawesome/webfonts/fa-solid-900.woff2`.

- [x] **2026-09-19: Product brochures rebuilt as HTML, and one palette everywhere.** The ten single-product brochures were the only thing in the business still set in **Helvetica**: they were built with fpdf2, which cannot read the variable Archivo woff2 the site and the catalogues use. At the owner's choice they now go through the same pipeline as the catalogues - HTML printed by headless Chrome - so all three read in **Archivo**. `brochure/brochure_html.py` replaces `brochure_layout.py` (deleted, ~600 lines of fpdf2 furniture); `PRODUCTS` and the php bridge are unchanged, so no copy moved. Same four pages, plus a proper spare-parts block and a quotation panel, and the same overflow guard the catalogues use (it passed on all forty pages).
  - **Three different reds became one.** Brochures were #C21807, catalogues #F03C02 (the website's *old* red) and the site #D40000 after the logo match earlier today. Everything is now **#D40000**, with the greys (`--soft`, `--rule`, `--panel`) aligned too. Both catalogues were regenerated.
  - Deleted along the way: `brochure_layout.py`, and a short-lived `tools/make_brochure_fonts.py` plus three static Archivo TTFs, which were only needed while the fpdf2 route was still on the table - HTML uses the variable woff2 directly.
- [x] **2026-09-19: Spare parts in the single-product brochures.** Page 4 now opens with a "Spare and wear parts" block above the gallery. Each part shows its photograph on a grey panel beside its name and what it fits, in a two-column grid. The images are transparent PNGs and the PDF image filter has no alpha, so they go through `cutout_image()` first - drawn directly they render as black boxes (the same trap the logo and the 30-years badge hit). The block reads `spares` / `spares_note` / `img` from `product-data.php` through the same php CLI bridge the Key specifications box already used (`site_specs` became `_site` + `site_specs` + `site_spares`), so the brochure cannot drift from the product page. A product with no `spares` key renders exactly as before, so only the two coal crushers show it today; adding the key to another product is all that is needed. The block's height is budgeted into the gallery's space calculation the same way the range index already was, so nothing runs into the quotation panel.
- [x] **2026-09-19: "At the battery" added to catalogue 1** as a facing pair, matching catalogue 2's A3 spread. It sits **after the vibrator screen and before the pusher**, opening the oven-machine group exactly as catalogue 2 does (pages 16-17), so both catalogues run in the same order. The photograph runs across both pages: `photo()` trims a crop to the frame's ratio about its own centre, which would re-centre each half and leave a jump at the fold, so `battery_halves()` cuts one 420x297 box in two and passes each half with `ratio=None`. The oven-machine table and the sizing note sit in a white panel over the lower right, as in catalogue 2. The contents page picks up the new section and the shifted numbers automatically. **Side effect: the catalogue is now 32 pages, a multiple of 4, so the standing "a folded booklet needs blanks" warning is gone.** Also fixed a figure that today's charging-car change had left stale in catalogue 2's panel ("the charging car's four mouths").
- [x] **2026-09-19: Power winch, haulage and crusher feed sizes.** Site and every PDF.
  - **Power winch:** every "Self-Locking" label is now plainly **"Worm Reducer Gearbox"** (hero spec, trust badge, technical table, brochure spec, catalogue callout and chart). On the owner's decision the **load-holding claim was dropped altogether**, not just the term: the intro's "acts as a natural brake system", the overview's power-failure sentence, the "cannot be back-driven" paragraph, the "Why a worm gearbox?" answer (now about ratio and torque) and the whole "Does it come with a brake?" FAQ, on the page and in the brochure. The winch now makes **no claim to hold the load if power fails** - consistent with removing the electro-magnetic brake earlier the same day. Its meta description fell to 135 chars and was brought back to 151.
  - **Haulage:** "C-channel base frame" -> **"MS Channel base frame"** (12 places).
  - **Feed size:** single disc 125 mm -> **150 mm**, double disc 150 mm -> **200 mm**, including the hub comparison table, both cross-links between the two machines, and the brochure summaries and "how it works" steps.
