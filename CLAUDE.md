...
## Other
- `brochure/*.py`: every PDF is HTML printed by **headless Chrome or Edge**, so the brochures, both catalogues and the website all use Archivo and one palette. Run them from the repo root because image paths are relative. Each script fails the build if any page's text runs into its footer, so shorten the copy rather than let a page spill.
  - **Keep the three `:root` blocks in step**: `brochure/brochure_html.py`, both `generate_range_catalogue*.py`, and `--c-action` in `assets/css/mtc.css`. The brand red is **#D40000** everywhere; the logo artwork itself stays #FF0000.
  - `generate_product_brochures.py` builds all ten from one `PRODUCTS` table (copy only); `brochure_html.py` holds the stylesheet, the page furniture and the Chrome printing. The copy is taken from the product pages, so update both when specs change.
  - Four pages: dark cover (product name, hero cutout, three headline figures), overview with a **Key specifications** row, the specification table, the feature list and the Buying information box, then how it works / applications / FAQ, then spare parts, gallery, the rest of the range and the quotation panel.
  - Two blocks are **not** copied into `PRODUCTS`: the script reads each page's four `specs` (icon, label, value) and its optional `spares` / `spares_note` from `product-data.php` through the `php` CLI (`SITE_SLUG` maps brochure keys to page slugs), so they always match the product page. Regenerate the brochures after changing either. A product with no `spares` key simply gets no spare parts block. Icon code points come from the site's own subset, `assets/fontawesome/css/icons.css`, and the glyphs from `assets/fontawesome/webfonts/fa-solid-900.woff2`.
  - Cover art comes from the home-page carousel images in `assets/img/slide/`, which are transparent cutouts; `Images.cutout()` trims to the alpha box and keeps it a PNG for the dark stage. A source with no alpha is flattened onto the cover colour instead.
  - `generate_range_catalogue.py` builds the full-range catalogue `Manual_Tools_Co_Catalogue.pdf` (32 A4 pages laid out as facing spreads: every product has its own two-page spread, an "At the battery" spread opens the oven machines, then the two specification charts). It reuses `PRODUCTS`/`BUYING_INFO` and the `product-data.php` specs, and the Archivo variable font in `brochure/fonts/`. Regenerate it whenever you regenerate the single-product brochures. **Leave a few seconds between the two catalogue scripts**: run back to back they collide over the build folder and one PDF comes out corrupt while both report success, so check each file's size against the size its script printed.
  ...
  - Transparent PNGs (the logo, the 30-years badge, the spare part cutouts) must be composited onto their background before they go into a PDF; drawn straight they render as black boxes.
